...
var total_words = 8  # Número total de palabras a encontrar (ajusta según sea necesario)
...
func _on_button_pressed(button_id: String):
	...
	check_sequence()  # Llama a la función para verificar la secuencia cada vez que se presiona un botón
...
func count_found_words():
	...
	if found_words >= total_words:
		...
		var count = total_words - found_words
